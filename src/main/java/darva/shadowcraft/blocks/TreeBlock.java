...
public class TreeBlock extends Block {
	...
	public TreeBlock(Material par2Material) {
		super(par2Material);
 ...
 {
     sideIcon = par1IconRegister.registerIcon("shadowcraft" + ":" + "log_oak");
     endIcon = par1IconRegister.registerIcon("shadowcraft" + ":" + "log_oak_top");
 }
	/* (non-Javadoc)
		...
		for (int length = 1; length < 100; length ++)
		{
			for (int curWidth = width *-1; curWidth <= width; curWidth++)
			{
				for (int curDepth = depth * -1; curDepth <= depth; curDepth++)
				{
					...
					{
					case 0: //south
						if (world.getBlock(x + curWidth, y + curDepth, z + length) == Main.TreeBlock)
						{
								return new Vector3(x+curWidth, y+curDepth, z+length);
						}
						continue;
					case 1: //West
						if (world.getBlock(x - length, y + curDepth, z + curWidth) == Main.TreeBlock)
						{
