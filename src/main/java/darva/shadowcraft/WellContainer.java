...
public class WellContainer extends Container {
	...
	class CloakSlot extends Slot
	{
		...
		public void onPickupFromSlot(EntityPlayer p_82870_1_, ItemStack p_82870_2_) {
			this.inventory.decrStackSize(3, 1); // Make sure you can't dupe the cloak.
		}

		@Override
		public boolean isItemValid(ItemStack itemstack) {
			if (itemstack.getItem() != Main.shadowArmor)
			{
				return false;
			}
			return true;
	...
	class FuelSlot extends Slot
	{
		...
		@Override
		public boolean isItemValid(ItemStack itemstack) {
			if (itemstack.getItem() != Main.itemShadow)
			{
				return false;
			}
			return true;
		...
		class RuneSlot extends Slot
		{
			...
			@Override
			public boolean isItemValid(ItemStack itemstack) {
				if (itemstack.getItem() != Main.runeItem)
				{
					return false;
				}
				return true;
		...
		public void onContainerClosed(EntityPlayer par1EntityPlayer) {
	        ...
	        if (!this.well.getWorldObj().isRemote)
	        {
	        	...
	        	for (int k = 0; k < 4; k++)
	        	{
	        		...
	        		{
						if (k < 3)
	        				par1EntityPlayer.entityDropItem(itemstack,0f);
						else
							itemstack.stackSize--;
